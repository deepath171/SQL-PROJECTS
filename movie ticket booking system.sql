...
-- Dumping data for table `SHOWRIMES`
...
--- Table structure for table'BOOKED SEATS'
...
-- Dumping data for table `BOOKED SEATS`
...
--- LIST ALL THEATERS IN CHENNAI

SELECT * FROM theaters WHERE location = 'Chennai';

--- LIST THE BOOKING ID WHICH BOOKED MORE THAN 2 SEATS

SELECT * FROM bookings WHERE seats_booked > 2;

--- JOIN QUERIES
---  SHOW MOVIES TITLE, SHOW DATE & TIME,THEATER NAME
...
--- LIST ALL BOOKINGS WITH CUSTOMER NAME AND MOVIE NAME
...
--- COUNT TOTAL BOOKINGS PER MOVIE
...
--- LAST 3 BOOKINGS MADE ( BASED ON BOOKING RATE )

SELECT * FROM bookings ORDER BY booking_date DESC LIMIT 3;


--- PREVENT OVERBOOKING BEYOND SCREEN CAPACITY
--- use case : stop insrting a ooking if total seats already booked for the show  exceeds screen capacity
...
--- PROCEDURE TO SEE AVAILABLE SEATS FOR S SHOW
...
--- SUB QUERY
--- CUSTOMERS WHO BOOKED MOVIES WITH RATING > 8.5
